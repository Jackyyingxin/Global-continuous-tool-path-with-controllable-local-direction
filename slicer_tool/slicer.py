...
class slicer:
    ...
    def incremental_slicing(self):
        ...
        for i in range(len(self.P)):
            ...
            A.extend(L[i])#A中只删除了没有被切割到的，关于L是只增加不减的
            t = 0
            while t < len(A):
    ...
    def contour_construction(self):
        ...
        for i in range(len(self.segments)):
            ...
            H = {}
            for seg in self.segments[i]:
                if np.linalg.norm(seg[0].coord-seg[1].coord) > 0.0001: #这是啥？？为啥要算线段的距离？
                    if seg[0].coord.tostring() in H:#判断键是否存在，若存在将该线段中不为键的点的坐标存入作为键的坐标中，不存在则创建新的键，如前。正反都存一次。
                    ...
                    if seg[1].coord.tostring() in H:
                        ...
                        H[seg[1].coord.tostring()].append(seg[0])#判断这种方法是否成功可以选择任意一条线段任意方向存得到的H是相同说明成功
                        #每次创建一个新的键是的第一个值就是这个键的坐标
