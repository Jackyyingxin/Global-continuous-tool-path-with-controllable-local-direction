...
from NEW_py_ver.TSP.POMO import reconstruction


class optimizer:

    def __init__(self, path, boundary, interval, radius):
        """
    ...
    def CacIp(self):
        ...
        # 注意，全都等于inf的时候，argmin会出错，所以要考虑这种情况。
        v1.cpu().numpy()
    ...
    def optimize(self, lamada):
        ...
        f = f + 0.3 * self.build_smooth_term()
        optimizer.zero_grad()
        f.backward()
        optimizer.step()
...
def load_data(file_path, device = "cuda"):
    ...
    for out in out_boundary:
        boundary = torch.from_numpy(np.loadtxt(out)).to(torch.float32).to(device)
    ...
    for inner in inner_boundary:
        inner = torch.from_numpy(np.loadtxt(inner)).to(torch.float32).to(device)
        if inner.shape[-1] == 2:
...
if __name__ == "__main__":
    ...
    path = torch.from_numpy(np.loadtxt(path_file_path)).to("cuda")
    # read boundary and path from xyz txt

    if path.shape[-1] == 2:
        padding_vector = torch.zeros((path.shape[0], 1))
